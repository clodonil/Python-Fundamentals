...
Se você já programa em outra linguagem, já está acostumado a declarar o tipo de dados que a variável vai receber. Isso é muito comum em `C`, `JAVA` e várias outras linguagens.
...
```
### 6.4 Tuple
As tuplas são uma sequência de itens, semelhante a uma lista. A diferença que a tuples são imultavél. Uma vez que a tupla são criadas, não podem ser modificadas.

As tuplas são usadas para dados que são protegidas contra gravação e por não serem modificadas dinamicamente, elas são usualmente mais rápidas que as listas. 
...
Nós podemos converter entre diferentes tipos de dados usando diferentes funções de conversão semelhantes a `int()`, `float()`, `str()`...etc.
...
## 7 Operadores
Vamos começar pelos operadores matemáticos, igualmente as outras linguagens de programação, as operações básicas de matemáticas são realizadas pelos operadores `+`, `-`, `/` e `*`. 
